...
test("random ID with seeds", function () {
  ...
  var random = _browser2.default.createWithSeeds(0);
...
test("random format", function () {
  expect(_browser2.default.id().length).toBe(17);
  expect(_browser2.default.id(29).length).toBe(29);

  var numDigits = 9;
  var hexStr = _browser2.default.hexString(numDigits);
  ...
  var randomFraction = _browser2.default.fraction();
  expect(randomFraction < 1.0).toBe(true);
  expect(randomFraction >= 0.0).toBe(true);

  expect(_browser2.default.secret().length).toBe(43);
  expect(_browser2.default.secret(13).length).toBe(13);
});

test("Alea is last resort", function () {
  var useGetRandomValues = !!(typeof window !== "undefined" && window.crypto && window.crypto.getRandomValues);
  expect(_browser2.default.alea === undefined).toBe(useGetRandomValues);
});

test("createWithSeeds requires parameters", function () {
  expect(function () {
    _browser2.default.createWithSeeds();
  }).toThrow();
});